...
export default function HospitalityCaregiverProfile({ navigation, route }) {
    ...
    async function getData() {
        let data = await getClientInfoWithJobId({ bidId: id, role: role }, 'hospitality');
        if (data?.error) {
            setUserInfo({});
        } else {
            setUserInfo(data);
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={{ width: '100%', marginTop: height * 0.22 }} showsVerticalScrollIndicator={false} >
                ...
                <Text
                    ...
                    onPress={() => navigation.navigate("HospitalityAllJobShiftList")}
                >
