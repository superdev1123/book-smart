...
import { aicAtom } from '../../../../context/HotelWorkProvider';
// Choose file
...
export default function HospitalityHotelWorkProfile({ navigation }) {
  ...
  const getData = async () => {
    setLoading(true);
    let result = await getUserInfo({ userId: aic }, 'hotel_user');
    if (!result?.error) {
      ...
      <ScrollView style = {styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.modal}>
          <View style={styles.authInfo}>
            ...
            <View>
              <Text style={constStyles.loginSubTitle}> Three Most Recent Experiences: <Text style={{color: 'red'}}>*</Text> </Text>
              <View style={{flexDirection: 'column', width: '100%', gap: 5}}>
                <TextInput
                  style={[constStyles.signUpinput, {width: '100%'}]}
                  placeholder=""
