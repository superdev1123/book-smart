...
import { View, StyleSheet, ScrollView, StatusBar, Alert, TouchableOpacity, TextInput, KeyboardAvoidingView, TouchableWithoutFeedback, Keyboard, Platform } from 'react-native';
...
export default function HospitalityHotelWorkApplyShift ({ navigation, route }) {
    const [content, setContent] = useState('');
    const [bidsubmit, setBidsubmit] = useState(false); 
    const { modalData, aic, firstName, lastName } = route.params;

    const handleBack = () => {
        navigation.navigate("HospitalityHotelWorkBookShift");
    };
