...
export default function HospitalityHotelWorkPhone ({ navigation }) {
  ...
  useEffect(() => {
    const getCredentials = async() => {
      const phoneNumber = await AsyncStorage.getItem('hotelWorkPhoneNumber') || '';
      if (phoneNumber !== '') {
  ...
  const handleSubmit = async () => {
    if (credentials.phoneNumber) {
      const response = await PhoneSms({phoneNumber: credentials.phoneNumber, email: email}, 'hotel_user');
      console.log(response)
      if (!response.error) {
        console.log('success');
        setVerifyPhone(credentials.phoneNumber);
        navigation.navigate('HospitalityHotelWorkPhoneVerify')
      } else {
        Alert.alert(
          'Failed!',
          `${response.error}`,
