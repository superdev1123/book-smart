...
/**
 * @param {Object} props
 * @param {{day:number,month:number,year:number,isFaded?:boolean}[]} props.calendarDays
 * @param {Record<string, {id?:string|number,shiftId?:string|number,color:string,label:string,time?:string}[]>} props.mockEvents
 * @param {(event:any)=>void} [props.setSelectedEvent]
 * @param {(show:boolean)=>void} [props.setShowEventModal]
 * @param {(date:Date)=>void} [props.setEventDate]
 * @param {(event:any,date:Date)=>void} [props.onEventPress]
 * @param {number} [props.footerHeight]  // pass your MFooter height (e.g. 80-100)
 * @param {boolean} [props.nestedScrollEnabled] // true if parent also scrolls
 */
