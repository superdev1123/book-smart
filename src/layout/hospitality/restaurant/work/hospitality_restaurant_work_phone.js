...
export default function HospitalityRestaurantWorkPhone ({ navigation }) {
  ...
  useEffect(() => {
    const getCredentials = async() => {
      const phoneNumber = await AsyncStorage.getItem('restaurantWorkPhoneNumber') || '';
      if (phoneNumber !== '') {
  ...
  const handleSubmit = async () => {
    if (credentials.phoneNumber) {
      const response = await PhoneSms({phoneNumber: credentials.phoneNumber, email: email}, 'restau_user');
      console.log(response)
      if (!response.error) {
        console.log('success');
        setVerifyPhone(credentials.phoneNumber);
        navigation.navigate('HospitalityRestaurantWorkPhoneVerify')
      } else {
        Alert.alert(
          'Failed!',
          `${response.error}`,
